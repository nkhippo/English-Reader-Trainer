...
| 5 | **軽い検索だけ差す** | たまに Cloze 的想起（**未実装・将来**） |
...
| 項目 | 内容 |
|------|------|
| バックグラウンド先読み | 次パッセージを事前生成（仕様 §5.3・未実装） |
| `CHUNK_JA_FALLBACKS` 撤去 | `ja_translation` バッチ完走後に GAS 内ハードコード辞書を削除 |
| 生成品質の検証 | 長期利用で Claude パッセージの自然さ・CEFR 遵守を確認 |
| Cloze（柱 5） | 軽い想起 UI — 未着手 |
...
1. **Phase 4  polish** — 先読み生成、生成品質モニタリング、fallback 辞書撤去
2. **Phase 5** — TTS + 再生 UI
3. **Phase 6** — ダッシュボード画面
...
*最終更新: 2026-06（Phase 4 実装・UX 改修・タイマー UI 復活後）*
